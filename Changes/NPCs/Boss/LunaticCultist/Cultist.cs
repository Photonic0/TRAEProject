                    ...
                    if (npc.ai[1] >= 4f && isCultist && (int)(npc.ai[1] - 4f) >= iceAttackDelay*(int)attackNumber)
                    {
                        ...
                        if (Main.netMode != 1)
                        {
                            ...
                            for (int n = 0; n < 1; n++)
                            {
                                ...
                                Projectile.NewProjectile(npc.GetSource_FromAI(), vector2.X, vector2.Y, vector3.X, vector3.Y, 464, iceDamage, 0f, Main.myPlayer, 0f, 1f);
                            }
                        }
                    }
                    npc.ai[1] += AttackSpeed(npc);
                    ...
                    if (npc.ai[1] >= 180f && isCultist && (int)(npc.ai[1] - 180f) >= fireballAttackDelay*(int)attackNumber)
                    {
                        ...
                        if (Main.netMode != 1)
                        {
                            ...
                            for (int num23 = 0; num23 < 1; num23++)
                            {
                                ...
                                Projectile.NewProjectile(npc.GetSource_FromAI(), vector5.X, vector5.Y, spinninpoint3.X, spinninpoint3.Y, 467, fireballDamage, 0f, Main.myPlayer);
                            }
                        }
                    }
                ...
                else if (npc.ai[0] == 4f)
                {
                    ...
                    if (isCultist && Main.netMode != 1)
                    {
                        if ((int)(npc.ai[1] - 20f) >= lightningAttackDelay * (int)attackNumber )
                        {
                            IllusionsAttack(npc, player);
                            attackNumber++;
                            Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center.X, npc.Center.Y - 100f, 0f, 0f, 465, lightningDamage, 0f, Main.myPlayer);
                        }
                    }
                    npc.ai[1] += AttackSpeed(npc);
                ...
                else if (npc.ai[0] == 5f)
                {
                    ...
                    else if (npc.ai[1] >= 30f && npc.ai[1] < 90f)
                    {
                        if (npc.ai[1] >= 30f && attackNumber < 1 && Main.netMode != 1 && isCultist)
                        {
                            ...
                            npc.ai[2] = Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center.X, npc.Center.Y, 0f, 0f, 490, 0, 0f, Main.myPlayer, 0f, npc.whoAmI);
            ...
            foreach (int item5 in list5)
            {
                ...
                if (Main.netMode != 1)
                {
                    ...
                    for (int num26 = 0; num26 < 1; num26++)
                    {
                        ...
                        Projectile.NewProjectile(nPC5.GetSource_FromAI(), vector6.X, vector6.Y, spinninpoint4.X, spinninpoint4.Y, 468, 18, 0f, Main.myPlayer);
                    }
                }
            }
        }
