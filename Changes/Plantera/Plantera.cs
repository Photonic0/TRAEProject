...
namespace TRAEProject.Changes.Plantera
{
    public class Plantera : GlobalNPC
    {
        ...
        void Movement(NPC npc, Player player)
        {
            ...
            if ((!Collision.CanHit(npc.Center + TRAEMethods.PolarVector(-20, rot), 1, 1, player.Center, 1, 1) || !Collision.CanHit(npc.Center + TRAEMethods.PolarVector(80, rot), 1, 1, player.Center, 1, 1)) || ((currentAtk == 1 || currentAtk == 6) && (player.Center - npc.Center).Length() > 700))
            {
        ...
        void SpitSeeds(NPC npc, Player player)
        {
            if (timer > 90)
            {
                ...
                if (timer > 90 + attackCounter * 20)
                {
                    attackCounter++;
                    float rot = (player.Center - npc.Center).ToRotation();
                    Projectile.NewProjectile(npc.GetProjectileSpawnSource(), npc.Center + TRAEMethods.PolarVector(50, rot), TRAEMethods.PolarVector((player.Center - npc.Center).Length() / 120f, rot), ProjectileID.SeedPlantera, npc.GetAttackDamage_ForProjectiles(26f, 22f), 0);
                }
            }
        }
        ...
        void ThornBall(NPC npc, Player player)
        {
            ...
            {
                thornBall = Main.projectile[Projectile.NewProjectile(npc.GetProjectileSpawnSource(), npc.Center, Vector2.Zero, ProjectileID.ThornBall, npc.GetAttackDamage_ForProjectiles(39f, 33f), 0)];
            ...
            if (thornBall != null)
            {
                ...
                else if (timer < thornBallStart + thornBallHold + thornBallRelease)
                {
                    ...
                    {
                        attackCounter++;
                        thornBall.velocity = TRAEMethods.PolarVector(12, rot);
                    }
        ...
        void Tentacles(NPC npc, Player player)
        {
            if (timer > 90)
            {
                if (timer > 90 + attackCounter * 24)
                {
                    ...
                    NPC tent = Main.npc[NPC.NewNPC((int)pos.X, (int)pos.Y, NPCID.PlanterasTentacle)];
        ...
        void Tentacles2(NPC npc, Player player)
        {
            if (attackCounter >= 0 && timer > 180 + attackCounter * 150)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    if (hooks[i] != null && hooks[i].active)
                    {
                        ...
                        Projectile p = Main.projectile[Projectile.NewProjectile(npc.GetProjectileSpawnSource(), hooks[i].Center, TRAEMethods.PolarVector((player.Center - hooks[i].Center).Length() / 120f, rot), ProjectileID.PoisonSeedPlantera, npc.GetAttackDamage_ForProjectiles(26f, 22f), 0)];
                        if (i == 0)
                        {
                            p.localAI[1] = 1;
                        }
            ...
            if (attackCounter == -1 && timer > 30)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    float rot = ((float)i / 6f) * ((float)Math.PI * 2f);
                    ...
                    hooks[i] = Main.projectile[Projectile.NewProjectile(npc.GetProjectileSpawnSource(), pos, TRAEMethods.PolarVector(1f, rot + (float)Math.PI), ProjectileType<PlanteraHook2>(), npc.GetAttackDamage_ForProjectiles(26f, 22f), 0)];
                }
            }
        }
        void Vines(NPC npc, Player player)
        {
            if (timer > 40 && attackCounter < 1)
            {
                ...
                for (int k = 0; k < 2; k++)
                {
                    ...
                    Projectile.NewProjectile(npc.GetProjectileSpawnSource(), pos, TRAEMethods.PolarVector(8, rot), ProjectileType<PlanteraHook>(), npc.GetAttackDamage_ForProjectiles(39f, 33f), 0);
                }
            }
        }
