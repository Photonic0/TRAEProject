    ...
    public abstract class WhipProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			if (Projectile.type == ProjectileType<AbsoluteZeroP>())
			{
				...
				for (int j = 0; j < 5; j++)
				{
					Dust dust5 = Dust.NewDustDirect(r5.TopLeft(), r5.Width, r5.Height, 16, 0f, 0f, 0, default, 1.2f);
					dust5.noGravity = true;
					dust5.velocity += value2 * 2f;
				}
				for (int k = 0; k < 2; k++)
				{
					Dust.NewDustDirect(r5.TopLeft(), r5.Width, r5.Height, DustID.Ice, 0f, 0f, 0, default, 0.8f).velocity += value2 * 2f;
				}
				for (int l = 0; l < 4; l++)
				{
