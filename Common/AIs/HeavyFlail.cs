...
namespace TRAEProject.Common.AIs
{
    public static class HeavyFlail
    {
		...
		public static void AI(Projectile Projectile, int throwTime = 10, float throwSpeed = 24f, float recoverDistance = 16f, float recoverDistance2 = 48f, int attackCooldown = 20)
		{
			...
			float num19 = throwSpeed * (float)throwTime;
			...
			switch ((int)Projectile.ai[0])
			{
				...
				case 1:
					{
						bool flag4 = Projectile.ai[1]++ >= (float)throwTime;
				...
				case 5:
					if (Projectile.ai[1]++ >= (float)num17)
					{
						Projectile.ai[0] = 6f;
						Projectile.ai[1] = 0f;
						Projectile.netUpdate = true;
